/**
 * 中文文案。
 *
 * 分组约定：按 UI 位置 / 关注点拆命名空间，避免单一巨大 namespace。
 *
 * 翻译策略：
 *   - 句段（脚本中一句 / 一行的录音单位）/ 录次（一次录音尝试）：
 *     用户视角 / UI 字符串全部本地化为这两个词
 *   - 行业 / UI 通用名（WAV / SRT / Dock / Dockview / Dark / Light / 等）
 *     保持原文——翻译反而不直观
 *   - 代码 / 注释里的标识符（types.ts 的 segmentsById 等）保持英文原状，
 *     与 i18n 显示文案解耦
 */
export const zhCN = {
  common: {
    cancel: '取消',
    confirm: '确认',
    close: '关闭',
    delete: '删除',
    save: '保存',
    ok: '好'
  },

  app: {
    title: 'Utterlane',
    tagline: '口播录音工作流'
  },

  menu: {
    file: '文件',
    file_new: '新建工程…',
    file_open: '打开工程…',
    file_close: '关闭工程',
    file_save: '保存',
    file_import: '导入文案…',
    file_export: '导出',
    file_export_wav: '导出音频（WAV）…',
    file_export_srt: '导出字幕（SRT）…',
    file_audit: '音频文件审计…',
    file_exit: '退出',
    edit: '编辑',
    edit_undo: '撤销',
    edit_redo: '重做',
    edit_undo_labeled: '撤销：{{label}}',
    edit_redo_labeled: '重做：{{label}}',
    edit_delete: '删除',
    view: '视图',
    view_reset_layout: '重置布局',
    view_dock_theme: 'Dock 主题（预览）',
    transport: '播放',
    transport_record: '录音',
    transport_rerecord: '重录',
    transport_play_segment: '播放当前句',
    transport_play_project: '播放项目',
    transport_stop: '停止',
    help: '帮助',
    help_user_guide: '使用说明',
    help_about: '关于 Utterlane',
    help_open_logs: '打开日志目录'
  },

  welcome: {
    new_project: '新建工程',
    new_project_hint: '从一段文案开始',
    open_project: '打开工程',
    open_project_hint: '选择现有工程目录',
    recent_projects: '最近工程',
    no_recent: '还没有最近工程',
    remove_recent: '从最近工程中移除'
  },

  segments: {
    col_order: '#',
    col_text: '文案',
    col_status: '状态',
    col_takes: '录次',
    col_duration: '时长',
    status_unrecorded: '未录制',
    status_recorded: '已录制',
    status_multi_take: '多录次',
    empty_title: '还没有任何句段',
    empty_hint: '把文案粘贴进来，每一行会被拆成一个句段',
    empty_action: '导入文案',
    search: '搜索',
    search_placeholder: '按文案内容过滤…',
    search_match_count: '{{count}} / {{total}}',
    batch_selected: '已选 {{count}} 条',
    batch_delete: '批量删除',
    tb_new: '新建句段（追加在末尾）',
    tb_insert_before: '在选中之前插入',
    tb_insert_after: '在选中之后插入',
    tb_delete_selected: '删除选中（含批量）',
    tb_clear_all: '清空所有句段',
    tb_import_script: '导入文案…',
    tb_find_replace: '查找 / 替换（Ctrl+F）',
    tb_paragraph_head: '将选中段标记为段首（再次点击取消）',
    tb_paragraph_head_locked: '首段恒为段首',
    find_placeholder: '查找…',
    replace_placeholder: '替换为…',
    replace_all: '全部替换',
    replace_done_title: '替换完成',
    replace_done_desc: '已替换 {{count}} 个句段中的内容',
    replace_none_title: '没有匹配项',
    replace_none_desc: '没有句段包含查找内容',
    ctx_insert_before: '在前插入空白句段',
    ctx_insert_after: '在后插入空白句段',
    ctx_mark_paragraph_head: '设为段首',
    ctx_unmark_paragraph_head: '取消段首',
    ctx_merge_prev: '合并到前一段',
    ctx_delete: '删除句段'
  },

  inspector: {
    unselected: '未选中句段',
    field_order: '顺序',
    field_text: '文案',
    field_paragraph: '段落',
    field_gap_after: '尾部间隔',
    paragraph_head_yes: '段首',
    paragraph_head_no: '段中',
    paragraph_first_locked_hint: '首段恒为段首',
    gap_after_follow_global: '跟随全局',
    gap_after_follow_global_hint:
      '勾选：本段间隔可被「应用默认间隔」覆盖。取消勾选：作为手动值，「应用默认间隔」会跳过本段。',
    gap_after_clear_hint: '清除间隔（恢复为「未设置」）',
    gap_after_last: '— （末段）',
    gap_after_last_hint: '末段没有「下一段」，尾部间隔无意义',
    take_duration_trimmed_hint: '节选后 {{effective}} / 原始 {{total}}',
    col_enable: '启用',
    col_take: '录次',
    col_trim_start: '起点 (ms)',
    col_trim_end: '终点 (ms)',
    col_duration: '时长',
    col_resize_hint: '拖动调整列宽 · 双击恢复默认',
    take_reveal_aria: '在文件管理器中定位',
    btn_play: '播放',
    btn_stop: '停止',
    btn_record: '录音',
    btn_stop_recording: '停止录音',
    btn_cancel: '取消',
    btn_rerecord: '重录',
    btn_delete_segment: '删除句段',
    btn_split: '拆分',
    btn_split_hint: '把光标停在文案中要拆分的位置，再点这里',
    btn_merge_prev: '合并到前一段',
    btn_paragraph_head: '段首',
    btn_paragraph_head_locked: '首段不可取消「段首」标记',
    takes_label: '录次',
    takes_count: '{{count}} 个',
    takes_empty: '还没有录音',
    take_item: '录次{{index}}',
    take_set_current: '设为当前',
    take_current: '当前',
    take_delete_aria: '删除录次',
    take_play_aria: '试听录次',
    level_label: '电平'
  },

  timeline: {
    row_segment: '句段',
    row_project: 'Project',
    btn_prev_segment: '上一句',
    btn_next_segment: '下一句',
    btn_prev_take: '上一个录次',
    btn_next_take: '下一个录次',
    btn_play_segment: '播放当前句',
    btn_play_from_head: '从段头开始播放',
    btn_play_from_playhead: '从游标位置播放',
    btn_stop_segment: '停止',
    btn_pause: '暂停',
    btn_resume: '继续',
    btn_record: '录音',
    btn_stop_recording: '停止录音',
    btn_rerecord: '重录（覆盖当前录次）',
    tb_apply_default_gaps: '应用默认间隔',
    tb_apply_default_gaps_hint:
      '把所有未手动设置过间隔的句段自动填充：句间 {{sentence}} ms / 段间 {{paragraph}} ms。已手动设置的不会被覆盖。',
    tb_reset_gaps_hint: '重置为默认间隔：强制把所有段（包括手动设置过的）都设回默认值。',
    tb_clear_auto_gaps_hint:
      '删除所有默认间隔：清空所有由「应用默认间隔」自动写入的间隔，手动设置的保留。',
    ctx_insert_before: '在前插入空白句段',
    ctx_insert_after: '在后插入空白句段',
    handle_resize_gap_hint: '拖拽调整与相邻段之间的空白间隔',
    zoom_slider_hint: '拖动缩放 · 双击恢复 1x · 时间轴上 Ctrl+滚轮也可缩放',
    zoom_h_slider_hint: '横向缩放 — 拖动调整时间轴密度 · 双击恢复 1x · 波形上 Ctrl+滚轮也可缩放',
    zoom_v_slider_hint:
      '纵向缩放 — 拖动调整振幅高度 · 双击恢复 1x · 波形上 Ctrl+Shift+滚轮也可缩放',
    btn_play_project_from_start: '从项目头开始播放',
    btn_play_from_current_segment: '从当前段开始播放',
    btn_play_project: '播放项目',
    btn_stop_project: '停止',
    btn_pause_project: '暂停',
    btn_resume_project: '继续',
    btn_speed_minus: '减速 0.25x',
    btn_speed_plus: '加速 0.25x',
    speed_select_hint: '当前 {{rate}}x · 选择档位或用 ± 步进',
    clip_unrecorded: '未录制',
    section_segment: '句段时间轴',
    section_project: '项目时间轴',
    segment_text_placeholder: '选中句段后在此编辑文案',
    waveform_unrecorded: '未录制',
    waveform_loading: '加载波形中…',
    waveform_error: '波形加载失败：{{message}}',
    trim_start_handle: '拖拽设置节选起点',
    trim_end_handle: '拖拽设置节选终点',
    trim_label: '节选',
    trim_full: '整段',
    trim_clear: '清除节选'
  },

  project_settings: {
    section_info: '工程信息',
    section_text: '文案',
    section_export_defaults: '默认导出设置',
    section_paths: '路径信息',
    field_title: '名称',
    field_sample_rate: '采样率',
    field_channels: '声道数',
    field_audio_format: '音频格式',
    field_subtitle_format: '字幕格式',
    field_segments_file: '句段文件',
    field_audios_dir: '音频目录',
    field_recommended_max_chars: '推荐最大字数',
    recommended_max_chars_placeholder: '不限制',
    recommended_max_chars_hint: '超过该数值的句段将被红色高亮提示，但不会阻止保存',
    section_gaps: '默认间隔',
    field_default_sentence_gap: '句间间隔',
    field_default_paragraph_gap: '段间间隔',
    channel_mono: 'Mono',
    channel_stereo: 'Stereo'
  },

  statusbar: {
    saved: '已保存',
    unsaved: '未保存',
    default_input: '默认输入设备',
    no_project: '无活动工程',
    playback_idle_unrecorded: '未录制',
    playback_idle_recorded: '已录制',
    playback_recording: '正在录音',
    playback_segment: '正在播放当前句',
    playback_segment_paused: '当前句已暂停',
    playback_project: '正在播放项目',
    playback_project_paused: '项目已暂停',
    background_none: '后台任务：无',
    segment_index: '句段{{index}} / {{total}}',
    take_index: '录次{{index}} / {{total}}',
    sample_rate: '{{khz}} kHz · {{channels}}',
    jump_to_project_settings_hint: '点击跳转到工程设置'
  },

  import_dialog: {
    title: '导入文案',
    description: '粘贴文案，每行会被拆分为一个句段（空行会被忽略）。',
    placeholder: '在这里粘贴你的文案…',
    will_generate: '将生成 <strong>{{count}}</strong> 条句段',
    overwrite_warning: '当前已有 {{count}} 条，导入会替换',
    btn_import: '导入',
    btn_import_count: '导入 ({{count}})',
    confirm_replace_title: '替换已有句段？',
    confirm_replace_description: '当前工程已有 {{count}} 条句段，导入会全部替换。',
    confirm_replace_btn: '替换'
  },

  confirm: {
    delete_segment_title: '删除这条句段？',
    delete_segments_title: '删除选中的 {{count}} 条句段？',
    clear_all_segments_title: '清空全部 {{count}} 条句段？',
    clear_all_segments_description: '此操作可以通过 Ctrl+Z 撤销，但会丢失所有录次引用。',
    close_recording_title: '正在录音，确定关闭吗？',
    close_recording_description: '当前录音将被丢弃。',
    close_recording_btn: '关闭并丢弃',
    close_unsaved_title: '还有未保存的改动，确定关闭吗？',
    close_unsaved_btn: '丢弃并关闭'
  },

  errors: {
    open_project_title: '无法打开工程',
    project_busy_title: '工程已被占用',
    project_busy_description: '已在另一个窗口中打开（PID {{pid}}）。请先关闭那个窗口后再试。',
    save_segments_title: '保存失败',
    save_segments_description: 'segments.json 写入出错：{{message}}',
    save_project_title: '工程信息保存失败',
    save_project_description: 'project.json 写入出错：{{message}}',
    recording_start_title: '无法开始录音',
    recording_stop_title: '停止录音失败',
    recording_stop_description:
      '录音过程中断：{{message}}\n\n这通常是麦克风被其他应用占用或设备临时断开。本次录音的音频可能已经丢失，请重试。',
    recording_persist_title: '录音落盘失败',
    recording_persist_description:
      '录音音频写入工程目录失败：{{message}}\n\n常见原因：磁盘空间不足、工程目录不可写、被杀毒软件 / OneDrive 等同步工具锁定。请检查工程目录权限后重试。',
    recording_device_missing: '录音设备已不可用，请在偏好设置里重新选择输入设备',
    play_missing_take_title: '无法播放',
    play_missing_take_description:
      '当前录次的音频文件已丢失，请打开「音频文件审计」修复或选择其他录次。'
  },

  export: {
    success_title: '{{kind}}导出成功',
    failure_title: '{{kind}}导出失败',
    skipped_count: '跳过 {{count}} 条未录制段',
    kind_wav: 'WAV 音频',
    kind_srt: 'SRT 字幕'
  },

  crash: {
    title: '应用程序遇到未处理的错误',
    description: '错误已写入日志。可以复制下方信息附在反馈中，或打开日志目录查看完整记录。',
    stack_label: '调用栈',
    copy_btn: '复制错误信息',
    open_logs_btn: '打开日志目录',
    copied: '错误信息已复制到剪贴板'
  },

  about: {
    title: '关于',
    version: 'v{{version}}',
    license_label: '本软件使用',
    license_suffix: '协议开源',
    diagnostics_label: '诊断信息',
    copy_btn: '复制',
    copy_tooltip: '复制版本与运行时信息（提交 bug 时附上）',
    copied: '已复制到剪贴板',
    third_party_label: '第三方库 / 许可证',
    third_party_count: '{{count}} 项'
  },

  user_guide: {
    title: '使用说明'
  },

  level_meter: {
    title: '电平'
  },

  countdown: {
    cancel_hint: '按 Esc 或点击屏幕取消'
  },

  text_editor: {
    too_long_hint: '文案过长，建议拆分'
  },

  history: {
    edit_text: '编辑文案',
    reorder: '调整句段顺序',
    delete_segment: '删除句段',
    delete_take: '删除录次',
    set_selected_take: '切换当前录次',
    import_script: '导入文案',
    split_segment: '拆分句段',
    merge_segment: '合并句段',
    delete_segments_batch: '批量删除句段',
    insert_segment: '新建句段',
    insert_segment_before: '在前插入句段',
    insert_segment_after: '在后插入句段',
    clear_segments: '清空句段',
    set_paragraph_start: '切换段首',
    replace_all: '全部替换',
    set_segment_gap: '调整段后间隔',
    apply_default_gaps: '应用默认间隔',
    reset_gaps_to_default: '重置为默认间隔',
    clear_auto_gaps: '清除自动间隔',
    set_take_trim: '调整录次节选'
  },

  audit_dialog: {
    title: '音频文件审计',
    rescan: '重新扫描',
    section_missing: '缺失录次',
    section_missing_hint: 'segments.json 引用了这些录次，但磁盘上找不到对应文件',
    section_missing_empty: '没有缺失的录次',
    section_orphans: '孤儿 WAV',
    section_orphans_hint: 'audios 目录里存在但没有任何句段引用的文件',
    section_orphans_empty: '没有孤儿文件',
    column_segment: '#',
    column_text: '文案',
    column_path: '期望路径',
    column_file: '文件',
    column_size: '大小',
    column_mtime: '修改时间',
    column_actions: '操作',
    btn_remap: '指定 WAV…',
    btn_save_as_take: '保存为录次…',
    btn_delete_orphan: '删除',
    save_as_take_dialog_title: '选择目标句段',
    save_as_take_dialog_desc: '把这个 WAV 转入哪个句段的录次列表？',
    inspector_missing_badge: '缺失',
    inspector_missing_tooltip: '文件缺失。打开 File → 音频文件审计 修复',
    bytes_kb: '{{value}} KB',
    bytes_mb: '{{value}} MB',
    toast_remap_success: '已恢复录次文件',
    toast_remap_failure: '恢复失败',
    toast_save_as_take_success: '已加入录次列表',
    toast_save_as_take_failure: '保存失败',
    toast_delete_success: '已移到回收站',
    toast_delete_failure: '删除失败'
  },

  export_dialog: {
    title: '导出音频',
    section_mode: '导出方式',
    mode_concat: '拼接为单个 WAV',
    mode_concat_hint: '所有句段按顺序合并到一个文件',
    mode_split: '每段单独导出',
    mode_split_hint: '每个句段一个 WAV，文件名带序号 + 文案前缀',
    section_format: '音频格式',
    label_sample_rate: '采样率',
    label_bit_depth: '位深',
    sample_rate_match_project: '跟随工程（{{rate}} Hz）',
    bit_depth_pcm16: '16-bit PCM（兼容性最好）',
    bit_depth_pcm24: '24-bit PCM（更高动态范围）',
    bit_depth_float32: '32-bit IEEE 浮点（DAW 友好）',
    section_effects: '后处理',
    label_silence_padding: '段间静音',
    silence_off: '不填充',
    silence_ms: '{{count}} 毫秒',
    silence_split_note: '拆分模式下每段独立成文件，段间静音不生效',
    label_peak_normalize: '峰值归一化',
    peak_off: '关闭',
    peak_minus_1: '-1 dB（接近最大）',
    peak_minus_3: '-3 dB（推荐）',
    peak_minus_6: '-6 dB（保留余量）',
    peak_minus_12: '-12 dB（充足余量）',
    btn_export: '导出'
  },

  tab_menu: {
    header_position: 'Tab 栏位置',
    position_top: '上方',
    position_bottom: '下方',
    position_left: '左侧',
    position_right: '右侧'
  },

  panel: {
    segments: '句段',
    inspector: '检查器',
    project_settings: '工程设置',
    segment_timeline: '句段时间轴',
    project_timeline: '项目时间轴',
    level_meter: '电平'
  },

  preferences: {
    menu_entry: '偏好设置…',
    title: '偏好设置',
    section_appearance: '外观',
    section_editor_theme: '编辑器配色',
    section_project_defaults: '新工程默认',
    section_recording: '录音',
    label_theme_preset: '预设主题',
    theme_preset_dark: '暗色',
    theme_preset_light: '亮色',
    theme_preset_highContrast: '高对比度',
    theme_overrides_label: '颜色调整（在预设基础上的覆盖）',
    theme_reset_overrides: '重置',
    theme_reset_overrides_hint: '清空所有自定义覆盖，回到纯预设状态',
    theme_token_overridden: '已在预设基础上修改',
    label_input_device: '输入设备',
    input_device_default: '（系统默认）',
    label_countdown: '录音前倒计时',
    countdown_off: '关闭',
    countdown_seconds: '{{count}} 秒',
    label_dock_theme: 'Dock 主题',
    label_font_scale: '界面缩放',
    label_language: '界面语言',
    label_segment_text_align: '句段时间轴文案对齐',
    label_inspector_text_align: '检查器文案对齐',
    label_sample_rate: '默认采样率',
    label_channels: '默认声道',
    font_scale_small: '紧凑',
    font_scale_default: '默认',
    font_scale_large: '舒适',
    font_scale_xlarge: '宽松',
    language_zh_cn: '简体中文',
    language_en_us: 'English',
    section_keyboard: '快捷键',
    kb_press_keys: '按下新键…（Esc 取消）',
    kb_unbound: '未绑定',
    kb_reset: '重置',
    kb_action_record: '录音',
    kb_action_rerecord: '重录（覆盖当前录次）',
    kb_action_playSegment: '播放当前句 / 暂停',
    kb_action_playProject: '播放项目 / 暂停',
    kb_action_prevSegment: '上一段',
    kb_action_nextSegment: '下一段',
    kb_action_stopOrCancel: '停止 / 取消'
  }
}

export type TranslationSchema = typeof zhCN
